...
func DeterministicGenesisStateCapella(t testing.TB, numValidators uint64) (state.BeaconState, []bls.SecretKey) {
	...
	beaconState, err := genesisBeaconStateCapella(context.Background(), deposits, uint64(0))
	if err != nil {
...
func genesisBeaconStateCapella(ctx context.Context, deposits []*ethpb.Deposit, genesisTime uint64) (state.BeaconState, error) {
	...
	return buildGenesisBeaconStateCapella(genesisTime, st)
}
...
func buildGenesisBeaconStateCapella(genesisTime uint64, preState state.BeaconState) (state.BeaconState, error) {
