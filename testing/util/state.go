...
// NewBeaconStateBadger creates a beacon state with minimum marshalable fields.
func NewBeaconStateBadger(options ...func(state *ethpb.BeaconStateBadger) error) (state.BeaconState, error) {
	pubkeys := make([][]byte, 512)
	for i := range pubkeys {
		pubkeys[i] = make([]byte, 48)
	}

	seed := &ethpb.BeaconStateBadger{
		BlockRoots:                 filledByteSlice2D(uint64(params.BeaconConfig().SlotsPerHistoricalRoot), 32),
		StateRoots:                 filledByteSlice2D(uint64(params.BeaconConfig().SlotsPerHistoricalRoot), 32),
		RandaoMixes:                filledByteSlice2D(uint64(params.BeaconConfig().EpochsPerHistoricalVector), 32),
		Validators:                 make([]*ethpb.Validator, 0),
	...
	var st, err = state_native.InitializeFromProtoUnsafeBadger(seed)
	if err != nil {
		return nil, err
	}
