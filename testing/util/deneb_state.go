...
func DeterministicGenesisStateDeneb(t testing.TB, numValidators uint64) (state.BeaconState, []bls.SecretKey) {
	...
	beaconState, err := genesisBeaconStateDeneb(context.Background(), deposits, uint64(0))
	if err != nil {
...
func genesisBeaconStateDeneb(ctx context.Context, deposits []*ethpb.Deposit, genesisTime uint64) (state.BeaconState, error) {
	...
	return buildGenesisBeaconStateDeneb(genesisTime, st)
}
...
func buildGenesisBeaconStateDeneb(genesisTime uint64, preState state.BeaconState) (state.BeaconState, error) {
