...
func TestMonitorRoutine(t *testing.T) {
	...
	// Wait for Logrus
	time.Sleep(1000 * time.Millisecond)
	wanted1 := fmt.Sprintf("\"Proposed beacon block was included\" balanceChange=100000000 blockRoot=%#x newBalance=256000000000 parentRoot=0x1c7093a757fb prefix=monitor proposerIndex=21 slot=1 version=1", bytesutil.Trunc(root[:]))
