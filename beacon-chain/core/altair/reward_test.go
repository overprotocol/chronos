...
func Test_BaseReward(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "active balance is 256over * target committee size",
			valIdx:    0,
		...
		{
			name:      "active balance is 256over * max validator per  committee size",
			valIdx:    0,
...
func Test_BaseRewardPerIncrement(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:          "active balance is 256over * 1m validators",
			epoch:         primitives.Epoch(0),
			activeBalance: params.BeaconConfig().MaxEffectiveBalance * 1e6,
			want:          951,
			wantReserve:   0,
