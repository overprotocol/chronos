...
func TestUpgradeToBadger(t *testing.T) {
	...
	mSt, err := badger.UpgradeToBadger(st)
	...
	require.Equal(t, preForkState.RewardAdjustmentFactor(), mSt.RewardAdjustmentFactor())
	...
	require.NoError(t, err)
	require.Equal(t, preVal.EffectiveBalance, mVal.EffectiveBalance)
	...
	require.NoError(t, err)
	require.Equal(t, 0, len(pendingDeposits))
